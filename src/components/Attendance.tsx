...
import { Clock, CheckCircle2, XCircle, AlertCircle, LogIn, LogOut, Search, Printer, MapPin, Coffee, Calendar as CalendarIcon, UserX, Clock9, ShieldQuestion, Fingerprint } from 'lucide-react';
...
interface AttendanceProps {
  initialViewMode?: 'daily' | 'monthly' | 'tracking';
  onViewModeChange?: (mode: 'daily' | 'monthly' | 'tracking') => void;
}

export default function Attendance({ initialViewMode = 'daily', onViewModeChange }: AttendanceProps) {
  ...
  const [viewMode, setViewMode] = useState<'daily' | 'monthly' | 'tracking'>(initialViewMode);
  ...
  const changeViewMode = (mode: 'daily' | 'monthly' | 'tracking') => {
    setViewMode(mode);
        ...
        <>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-4 md:p-6 rounded-[24px] border border-slate-100 dark:border-slate-800 shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div className="flex bg-slate-100/80 dark:bg-slate-800/80 p-1.5 rounded-xl no-print shadow-inner gap-1">
                <button
                          ...
                          <SelectTrigger className={`w-40 h-9 rounded-full text-[11px] font-black tracking-wide border transition-all shadow-sm cursor-pointer hover:brightness-105 ${getStatusColorClasses(effectiveStatus === '' ? undefined : effectiveStatus)}`}>
                            <SelectValue placeholder="اختر الحالة">
                              <div className="flex items-center gap-2">
                                {effectiveStatus ? statusIcons[effectiveStatus] : <Fingerprint size={14} className="text-slate-400" />}
